// https://trpc.io/docs/server/subscriptions#example-with-zod
...
>(opts: {
  ...
  return z
    .custom<AsyncGenerator<Tracked extends true ? TrackedEnvelope<TYieldIn> : TYieldIn, TReturnIn>>(val =>
      isAsyncIterable(val),
    )
    ...
    AsyncGenerator<Tracked extends true ? TrackedEnvelope<TYieldIn> : TYieldIn, TReturnIn, unknown>,
    any,
    AsyncGenerator<Tracked extends true ? TrackedEnvelope<TYieldOut> : TYieldOut, TReturnOut, unknown>
  >;
}
